...
열어야 하는 파일이 한 폴더 안에 모이는 편이 이어받기 쉽다고 판단했습니다.
...
관심 상태가 세 화면에서 어긋나지 않아야 한다는 것이 이 과제의 핵심 요구사항입니다.
그래서 **관심 목록을 단 하나(`favoritesProvider`)만 두고 세 화면이 모두 그것을 보게** 했습니다.
화면마다 별도의 상태를 두고 맞추는 코드를 쓰지 않았기 때문에, 동기화가 어긋날 자리가
아예 없습니다.

Riverpod을 고른 구체적인 이유는 세 가지입니다.

1. **`AsyncValue`.** 시세 · 검색 · 일별 시세가 모두 비동기입니다. 로딩 · 에러 · 데이터를
   각 화면에서 `bool isLoading` / `String? error`로 따로 들고 있으면 조합이 늘어날수록
   빠뜨리기 쉽습니다. `AsyncValue` 하나로 세 상태를 같은 방식으로 다뤘습니다.
   특히 **다시 조회하는 동안 직전 값을 유지**해 주기 때문에, 관심 종목을 추가했을 때
   기존 행은 그대로 두고 새 행만 스켈레톤으로 보이게 하는 동작이 그냥 따라옵니다.
2. **`select`.** 검색 결과의 별 아이콘은 `favoritesProvider.select(...)`로 자기 종목의
   관심 여부만 구독합니다. 다른 종목을 등록해도 그 행만 다시 그려집니다.
3. **`family` + 캐시.** 일별 시세는 `(종목, 기간)`이 키입니다. `family`로 두면 기간 탭을
   오갈 때 같은 키가 같은 요청으로 취급되어, 리포지토리 캐시와 자연스럽게 맞물립니다.

코드 생성(`riverpod_generator`)은 쓰지 않았습니다. provider가 10개 남짓이라 생성기를
붙여 얻을 이득보다, 저장소를 받은 사람이 `build_runner` 없이 바로 읽고 돌릴 수 있는
편이 낫다고 판단했습니다.
...
캔들 차트를 지원하면서 토큰 색과 시안의 여백까지 통제할 수 있는 패키지가 마땅치
않았습니다. 패키지를 쓰면 색·두께·여백을 맞추려고 옵션을 찾는 시간이, 직접 그리는
시간보다 길어질 것 같았습니다. `lib/ui/detail/widgets/candle_chart.dart` 한 파일(약 130줄)
입니다.
...
- **실기기 · 에뮬레이터 구동 확인.** 작업 환경에 Visual Studio C++ 툴체인과 Android AVD가
  없어 데스크톱 빌드와 에뮬레이터를 띄우지 못했습니다. 화면은 시안과 같은 크기의 위젯
  테스트로, 데이터는 실제 endpoint 요청으로 확인했습니다.
